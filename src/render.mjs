...
export function renderHome(data) {
  ...
  const races = data.races.map((race) => {
    ...
    const cards = candidates.map((candidate) => `<article class="candidate-card">
      <div class="candidate-top">${portrait(candidate)}<div><h3>${escapeHtml(candidate.name)}</h3><p class="party">${escapeHtml(candidate.party)}</p><p>${escapeHtml(candidate.summary)}</p></div></div>
      <div class="candidate-details"><p class="candidate-age"><strong>Age:</strong> ${escapeHtml(candidate.age || 'Not listed')}</p><h4>Background</h4>${(candidate.biography || []).map((item) => `<p>${escapeHtml(item.text)} ${citations(item.sourceIds, sources)}</p>`).join('')}<h4>Key positions</h4>${(candidate.positions || []).map((item) => `<p><strong>${escapeHtml(item.topic)}:</strong> ${escapeHtml(item.text)} ${citations(item.sourceIds, sources)}</p>`).join('')}${candidate.record?.length ? `<h4>Public record</h4><ul>${candidate.record.map((item) => `<li>${escapeHtml(item.text)} ${citations(item.sourceIds, sources)}</li>`).join('')}</ul>` : ''}<a class="read-more" href="candidates/${candidate.id}/index.html">Open full source list</a></div>
      </article>`).join('');
