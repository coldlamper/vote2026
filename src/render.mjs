...
export function renderHome(data) {
  ...
  const races = data.races.map((race) => {
    ...
    const cards = candidates.map((candidate) => `<article class="candidate-card">
      <a class="card-link" href="candidates/${candidate.id}/index.html">
        ${portrait(candidate)}<div><p class="party">${escapeHtml(candidate.party)}</p><h3>${escapeHtml(candidate.name)}</h3><p>${escapeHtml(candidate.summary)}</p><span class="read-more">View profile</span></div>
      </a></article>`).join('');
